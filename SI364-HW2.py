...
def doubleView():
    ...
    <html>
    <body>
    <form method="GET" action="http://localhost:5000/result-question">
...
def result_double():
    if request.method == 'GET':
        ...
        return str("Double your favorite number is ") + str(number)
...
def nameBackwards():
    ...
    <html>
    <body>
    <form method="GET" action="http://localhost:5000/result-yourname">
        <label>Enter your name:
            <input name="yourName" value="" type="text">
        </label></br>
        <input type="submit" value="Submit">
    </form>
    </body>
...
@app.route('/result-yourname', methods = ['GET', 'POST'])
def result_nameBackwards():
    if request.method == 'GET':
        new_result = request.args
        text = str(new_result['yourName'])
        text_backwards = text[::-1]
        text_lower = text_backwards.lower()
        return str("your name backwards is ") + text_lower.title()
